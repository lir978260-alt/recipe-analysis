...
def validate_username(username: str) -> tuple[bool, str]:
    ...
    if not USERNAME_RE.match(u):
        return False, "账号需为 3-32 位字母、数字、下划线、连字符或中文 / Username must be 3-32 chars (letters, digits, _ - or Chinese)"
    return True, ""
...
def validate_password(password: str) -> tuple[bool, str]:
    ...
    if classes < 2:
        return False, "密码需至少包含大小写字母、数字、符号中的两类 / Password needs at least 2 of: lowercase, uppercase, digit, symbol"
    return True, ""
...
theme_colors = {
    "🍵 抹茶绿 (Matcha Green)": {
        ...
        "PILL_BG": "#3d7355", "SIDE_TOP": "#4a7a52", "SIDE_BOT": "#8bb389", "PAGE_TOP": "#dcecd2", "PAGE_BOT": "#bcd9b8"
    },
    "🌊 海洋蓝 (Ocean Blue)": {
        ...
        "PILL_BG": "#2e6b8e", "SIDE_TOP": "#2d5b7a", "SIDE_BOT": "#82b0cb", "PAGE_TOP": "#dcedf3", "PAGE_BOT": "#b6dbe9"
    },
    "🌸 樱花粉 (Sakura Pink)": {
        ...
        "PILL_BG": "#b0567a", "SIDE_TOP": "#a85878", "SIDE_BOT": "#e3acc1", "PAGE_TOP": "#fbe9ef", "PAGE_BOT": "#f5cdda"
    },
    "☕ 奶茶棕 (Mocha Brown)": {
        ...
        "PILL_BG": "#8a5a32", "SIDE_TOP": "#7a4a22", "SIDE_BOT": "#c69c78", "PAGE_TOP": "#fbeadd", "PAGE_BOT": "#f1cba9"
    }
}
...
.stApp {{ background: linear-gradient(160deg, {PAGE_TOP} 0%, {PAGE_BOT} 100%) !important; color: {TEXT_MAIN} !important; }}
...
@media screen and (max-width: 1200px) {{
    ...
    div[data-testid="column"]:has(.side-card) div[data-testid="stButton"] > button {{ font-size: 0.92rem !important; min-height: 80px !important; margin-bottom: 16px !important; }}
    div[data-testid="column"]:has(.pill-btn) div[data-testid="stButton"] > button {{ padding: 0.4rem 1rem !important; font-size: 0.92rem !important; }}
}}
...
def _submit_vote(dish_name: str):
    ...
    try:
        ...
        if exist:
            ...
            supabase.table("dish_ranking").insert({"dish_name": clean_name, "votes": 1, "voted_by": [st.session_state.user]}).execute()
            st.session_state["_dish_q"] = ""
            st.rerun()
    except Exception:
        st.error("操作失败 / Operation failed")
                ...
                with st.container(border=True):
                    ...
                    if st.button(f"👍 {t['like']} ({int(r.get('likes', 0))})", key=f"like_btn_{r.get('id', 'temp')}", disabled=(not st.session_state.user or has_liked)):
                        if not st.session_state.user:
                            ...
                            try:
                                cur = supabase.table("comments").select("likes, liked_by").eq("id", r.get("id")).execute().data
                                cur_likers = cur[0].get("liked_by") if cur and isinstance(cur[0].get("liked_by"), list) else []
                                if st.session_state.user in cur_likers:
                                    ...
                                    supabase.table("comments").update({"likes": cur_likes + 1, "liked_by": cur_likers}).eq("id", r.get("id")).execute()
                                    st.rerun()
                            except Exception:
                                st.error("操作失败 / Operation failed")
...
def dlg_signup():
    ...
    st.caption("账号 3-32 位；密码 8-128 位且至少含两类字符 / Username 3-32 chars; password 8-128 chars, 2+ char types")
    a, b = st.columns(2)
    if a.button(t["close"]): st.rerun()
...
def render_home():
    ...
    with main:
        if st.session_state.user:
            ...
            with top[1]:
                st.markdown('<div class="pill-btn"></div>', unsafe_allow_html=True)
                if st.button(t["login"], key="home_login", use_container_width=True): dlg_login()
            with top[2]:
                st.markdown('<div class="pill-btn"></div>', unsafe_allow_html=True)
